    ...
    const fetchSchedule = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/scheduled-classes/instructor/6811d0540f79d80d29592792",
        )
        ...
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            {timeSlots.map((timeSlot) => (
              <tr key={timeSlot}>
                <td className="border p-2 bg-gray-50">{timeSlot}</td>
                {days.map((day) => {
                  const classes = groupedClasses[day][timeSlot]
                  return (
                    <td key={`${day}-${timeSlot}`} className="border p-2">
                      {classes.map((cls) => (
                        <div key={cls.id} className="mb-1 p-2 bg-blue-50 rounded">
                          <div className="font-medium text-sm">{cls.course?.courseName || "No course"}</div>
                          <div className="text-xs text-gray-600">{cls.course?.courseCode || ""}</div>
                        </div>
                      ))}
                    </td>
                  )
